...
* filename：FileManagerment.java
* Author：郭靖东
* Version：1.1
* Date：2015/07/31
* Description：This java file is used for uploading the file to the cloud, not used now.
* Modify history：
...
import hpecl.uclass.uti.TCPClientForPF;
...
public class FileManagerment extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getFileDir(rootPath);
		fAdapter = new FileListAdapter(FileManagerment.this, items, paths);
		lv.setAdapter(fAdapter);
		Intent intent = getIntent();
		mm = intent.getStringExtra("mm");
		
		destIP = mm.split(";")[2];
		Log.i("guo ", "tcpip     "+ destIP);
...
//							uploadDepartmentTime(file);
							Log.i("sdfa", "0103   "+ "tcp client start");
							TCPClientForPF tcpClient = new TCPClientForPF(destIP,Constant.PORTFORPF,file.toString());
							tcpClient.start();
...
//				
			...
			String[] menu = { "发送文件" };
			new AlertDialog.Builder(FileManagerment.this).setTitle("你要做什么?")
...
//										tv.setText(""+uploadURL);
											...
											Toast.makeText(FileManagerment.this, "文件已存在", Toast.LENGTH_SHORT).show();
										}
									} catch (JSONException e) {
										// TODO Auto-generated catch block
										e.printStackTrace();
