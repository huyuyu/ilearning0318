...
* filename：AskQuestion.java
* Author：郭靖东
* Version：1.1
* Date：2015/09/03
* Description：This java file is used for submitting the question to the website.
* Modify history：
* 郭靖东,create file,2015/09/03  
*/
package hpecl.uclass.ilearning;

import hpecl.uclass.analysis.JsoupAnalysisLogin;
import hpecl.uclass.form.FormSubmitAskQuestion;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		myHandler = new Handler() {  
	          public void handleMessage(Message msg) {   
	               switch (msg.what) {   
	                    case 1:   
	                         Toast.makeText(AskQuestion.this, info, Toast.LENGTH_SHORT).show();	                         
	                         break;   
	               }   
	               if(info.contains("成功")){
		               finish();
		...
		submit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(title.equals("") && 
					...
					Toast.makeText(AskQuestion.this, "不能都为空，请重新输入！", Toast.LENGTH_SHORT).show();
					...
					new Thread(new Runnable() {
						...
						public void run() {
							...
							String back = FormSubmitAskQuestion.post(Constant.Sharing0A701, cookie, title, keyboard, smallText, writer);
							JsoupAnalysisLogin jal = new JsoupAnalysisLogin();
							info = (String) jal.analysisInfo(back);
							Message message = new Message();   
		                    message.what = 1;   
		                      
		                    AskQuestion.this.myHandler.sendMessage(message);
						}
					}).start();
				}
